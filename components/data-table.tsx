...
export function DataTable({ tableName, data, columns, onFilter }: DataTableProps) {
  ...
  const handleMouseDown = (index: number) => {
    ...
    setSelectedCell(null); // clear cell selection
  };
  ...
  const handleCellClick = (rowIndex: number, colKey: string) => {
    if (colKey === columns[0]) {
      // First column clicked: select whole row
      ...
      if (e.ctrlKey && e.key === "c") {
        if (selectedCell) {
          ...
          const copiedText = [...selectedRows]
            .map((rowIndex) => columns.map((col) => sortedData[rowIndex][col]).join("\t"))
            .join("\n");
  ...
  }, [selectedCell, selectedRows, sortedData, columns]);
                  ...
                  >
                    {columns.map((column) => {
                      ...
                      return (
                        <TableCell
                          key={column}
                          className={`py-1 text-black border border-black ${
                            isSelectedCell ? "bg-green-100" : ""
                          }`}
                          onClick={() => handleCellClick(i, column)}
                        >
                          {row[column]}
                        </TableCell>
                      );
